...
public abstract class GenericEntityDao<T, I extends Serializable> {
  ...
  /*  public
    ...
    /**
     * created by hzj 2016.8.22
     * 根据intl类型的参数删除实体
     * @param propertyName
     * @param value
     * @return
     */
    public boolean deleteByIntProperty(String propertyName,int value){
        String hql="delete from "+entityClass().getName()+" model where model."+propertyName+"=:PROPERTY_VALUE";
        Query q=this.getCurrentSession().createQuery(hql).setParameter("PROPERTY_VALUE",value);
